---
title: rest pack
description: Cloud-mode-without-plugin reads — 26 tools backed by the Figma REST API. Six are write-gated.
---
...
### Webhooks (3 write-gated)

Backed by Figma's webhooks v2 API. Reads list/inspect; writes create/update/delete subscriptions. The MCP server does not host the endpoint that webhook payloads post to — the user's app does.

| Tool | What it does |
| ---- | ------------ |
| [list_team_webhooks](/docs/tools-reference/rest/list_team_webhooks) | Read all webhooks owned by a team. |
| [get_webhook](/docs/tools-reference/rest/get_webhook) | Read a single webhook by id. |
| [get_webhook_requests](/docs/tools-reference/rest/get_webhook_requests) | Read recent delivery attempts for a webhook. |
| [create_webhook](/docs/tools-reference/rest/create_webhook) | **Write-gated** — register a webhook for a team + event type. |
| [update_webhook](/docs/tools-reference/rest/update_webhook) | **Write-gated** — update endpoint / passcode / status / description. |
| [delete_webhook](/docs/tools-reference/rest/delete_webhook) | **Write-gated** — remove a webhook subscription. |
